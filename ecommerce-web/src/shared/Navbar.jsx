...
const Navbar = () => {
  ...
  const isAuthPage = ["/login", "/signup"].includes(location.pathname); // Check if it's the login or signup page
  ...
  // Reset hovered state when on auth pages
  useEffect(() => {
    if (isAuthPage) {
      setHovered(false);
    }
      ...
      <nav className="container mx-auto flex justify-between items-center px-4 md:py-2 pt-3 pb-2">
        ...
        <div className="text-lg text-alofa-pink sm:flex items-center gap-4 hidden">
          {!isAuthPage ? (
            ...
            // Optionally, show a link back to Home or any other specific links on auth pages
            <Link
              to="/"
              className="hover:text-pink-700 flex items-center gap-2"
            >
      ...
      {!isAuthPage && (
        ...
        >
          <div className="p-4 h-full flex flex-col">
            <h1 className="text-2xl text-alofa-pink font-bold mb-4">
              Cart Overview
            </h1>
